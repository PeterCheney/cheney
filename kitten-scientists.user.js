...
var run = function() {
	...
	Engine.prototype = {
		...
		reset: function() {
			...
			if (game.resPool.get("timeCrystal").value > 1e4 && game.religion.meta[1].meta[5].val) {
			...
			if (!options.auto.autoparagon.items.infinite.enabled && options.auto.autoparagon.enabled) {
				if (game.bld.get("workshop").on > 0 && game.science.get("construction").researched) {
					...
					if (game.bld.get("reactor").on == 160) {
						if (game.resPool.get("plate").value > 1e7 && options.auto.craft.items.plate.enabled) {
							$("#toggle-plate")[0].click();
						}

						if (game.bld.get("steamworks").on == 92 && game.bld.get("temple").on == 150 && game.resPool.get("compedium").value > 1e6 && game.resPool.get("blueprint").value > 1e6 && options.auto.craft.items.blueprint.enabled) {
							options.auto.craft.items.blueprint.enabled = false;
				...
				if (game.science.meta[0].meta[41].researched && options.auto.build.items.mansion.limited < 999) {
					if (game.tabs[6].planetPanels.length && game.tabs[6].planetPanels[0].children[0].model.on > 12) {
		...
		upgrade: function() {
			...
			if (upgrades.buildings.enabled) {
				var pastures = (game.bld.getBuildingExt('pasture').meta.stage === 0) ? game.bld.getBuildingExt('pasture').meta.val :
					0;
				var aqueducts = (game.bld.getBuildingExt('aqueduct').meta.stage === 0) ? game.bld.getBuildingExt('aqueduct').meta
					.val : 0;
		...
		hunt: function() {
			...
			if (options.auto.options.items.hunt.subTrigger <= manpower.value / manpower.maxValue) {
				...
				for (var out in aveOutput) {
					...
					trueOutput[out] = (res.maxValue > 0) ? Math.min(aveOutput[out] * huntCount, Math.max(res.maxValue - res.value,
						0)) : aveOutput[out] * huntCount;
	...
	CraftManager.prototype = {
		...
		getValueAvailable: function(name, all, typeTrigger) {
			...
			if ('catnip' === name) {
				var pastures = (game.bld.getBuildingExt('pasture').meta.stage === 0) ? game.bld.getBuildingExt('pasture').meta
					.val :
					0;
				var aqueducts = (game.bld.getBuildingExt('aqueduct').meta.stage === 0) ? game.bld.getBuildingExt('aqueduct')
					.meta
					.val : 0;
	...
	TradeManager.prototype = {
		...
		getAverageTrade: function(race) {
			...
			for (var s in race.sells) {
				...
				var mean = 0;
				var tradeChance = (race.embassyPrices) ? item.chance * (1 + game.getLimitedDR(0.01 * race.embassyLevel, 0.75)) :
					item.chance;
				if (race.name == "zebras" && item.name == "titanium") {
					...
					mean = (normBought + 1.25 * normBonus) * item.value * rRatio * sRatio * tRatio * 100;
				}
